...
$id   = (int)($_GET['id'] ?? 0);
$post = $mysqli->query("SELECT * FROM home_road_KURU_posts WHERE id=$id")->fetch_assoc();
...
$('#form_rb_edit').on('submit',function(e){
  ...
  function doSave(thumbUrl){
    $.post('ajax_save_road.php',{road:'KURU',id:<?= $id ?>,title:title,content:content,thumbnail:thumbUrl,is_secret:isSecret}).done(function(d){
      if(d.success) location.hash='#/road_KURU';
