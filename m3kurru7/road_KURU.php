...
<div class="container_roadbee">
  <div class="roadbee-header">
    <?php if ($isAdmin): ?>
    <a href="#/road_KURU/upload" class="btn btn-primary">+</a>
    <?php endif; ?>
  </div>
  <div class="roadbee-feed">
    <?php
    $secretWhere = $isGuest ? ' WHERE is_secret = 0' : '';
    $result = $mysqli->query("SELECT id,title,content,collapsed,is_secret FROM home_road_KURU_posts$secretWhere ORDER BY id DESC");
    ...
    <div class="rb-post" id="rb-post-<?= $row['id'] ?>" data-id="<?= $row['id'] ?>">
      <div class="rb-post-header">
        <div class="rb-post-left">
          ...
          <button class="rb-collapse-btn" onclick="rbToggle(<?= $row['id'] ?>,'KURU')">
            <i class="fa-solid <?= $isCollapsed ? 'fa-angle-down' : 'fa-angle-up' ?>"></i>
          </button>
        </div>
        <?php if ($isAdmin): ?>
        <div class="rb-post-right">
          <a href="#/road_KURU/edit/<?= $row['id'] ?>" class="btn btn-secondary btn-sm">수정</a>
          <button class="btn btn-danger btn-sm" onclick="rbDel(<?= $row['id'] ?>,'KURU')">삭제</button>
        </div>
        <?php endif; ?>
      </div>
      <div class="rb-post-body" id="rb-body-<?= $row['id'] ?>"<?= $isCollapsed ? ' style="display:none"' : '' ?>>
        ...
        <div class="rb-post-comments">
          ...
          <div class="rb-comment-form">
            ...
            <button class="btn btn-primary" onclick="rbCmtSubmit(<?= $row['id'] ?>,'KURU')">등록</button>
          </div>
        </div>
      </div>
    </div>
...
(function(){
  ...
  function loadComments(postId){
    $.get('ajax_get_road_comments.php',{road:'KURU',id:postId},function(data){
      var $list=$('#rb-cmt-'+postId).empty();
      data.items.forEach(function(c){
        var canDel = data.is_admin || c.is_mine;
        var delBtn = canDel ? '<button class="rbc-del" data-id="'+c.id+'" data-post="'+postId+'" data-road="KURU">✕</button>' : '';
        var editBtn = c.is_mine ? '<button class="rbc-del rbc-edit" data-id="'+c.id+'" data-post="'+postId+'" data-road="KURU" data-content="'+escH(c.content)+'" style="margin-right:10px;font-size:18px;position:relative;top:3px;">*</button>' : '';
    ...
    $btn.text('수정').removeAttr('onclick').off('click').on('click',function(){
      ...
      $.post('ajax_update_road_comment.php',{road:road,id:id,content:newContent}).done(function(d){
        if(d.success){
          ...
          $btn.text('등록').off('click').attr('onclick','rbCmtSubmit('+postId+',\'KURU\')');
          loadComments(postId);
