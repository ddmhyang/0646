...
$stmt = $mysqli->prepare('SELECT id,title,thumbnail,is_private,created_at FROM `home_gallery_Dress` ORDER BY id DESC');
$stmt->execute();
$posts = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
?>
<div class="container_gallery_Dress">
<div class="gallery-header">
  <h2>Dress</h2>
  <?php if ($is_admin): ?>
  <a href="#/gallery_Dress/upload" class="btn">글쓰기</a>
  <?php endif; ?>
</div>
<?php if (empty($posts)): ?>
...
<div class="gallery-grid">
  ...
  <div class="gallery-card" onclick="location.hash='#/gallery_Dress/detail/<?= $p['id'] ?>'">
