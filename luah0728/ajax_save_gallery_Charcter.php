...
if ($id > 0) {
    $chk = $mysqli->prepare('SELECT thumbnail, password_hash FROM `home_gallery_Charcter` WHERE id=? LIMIT 1');
    $chk->bind_param('i', $id);
    $chk->execute();
    ...
    if ($pw_hash !== null) {
        $stmt = $mysqli->prepare('UPDATE `home_gallery_Charcter` SET title=?,content=?,thumbnail=?,is_private=?,password_hash=?,updated_at=NOW() WHERE id=?');
        $stmt->bind_param('sssisi', $title, $content, $thumbnail, $is_private, $pw_hash, $id);
    } else {
        $stmt = $mysqli->prepare('UPDATE `home_gallery_Charcter` SET title=?,content=?,thumbnail=?,is_private=?,updated_at=NOW() WHERE id=?');
    ...
    $stmt = $mysqli->prepare('INSERT INTO `home_gallery_Charcter` (title,content,thumbnail,is_private,password_hash) VALUES (?,?,?,?,?)');
