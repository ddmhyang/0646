...
if ($post['is_private'] && !$is_admin) {
    $sess_key = 'post_access_Dress_' . $id;
    if (!isset($_SESSION[$sess_key]) || $_SESSION[$sess_key] < time()) {
        ...
        echo '  $.post("ajax_verify_password_Dress.php",{id:id,password:$("#pw_input").val()})';
...
?>
<div class="container_gallery_Dress_detail">
<div class="post-header" style="display:flex;align-items:center;justify-content:space-between;">
  ...
  <div style="display:flex;gap:8px;">
    <a href="#/gallery_Dress/edit/<?= $post['id'] ?>" class="btn">수정</a>
...
<div class="post-actions">
  <a href="#/gallery_Dress" class="btn btn-list">목록</a>
</div>
<?php if ($is_admin): ?>
<script>
function delPost(id) {
  if (!confirm('삭제하시겠습니까?')) return;
  $.post('ajax_delete_gallery_Dress.php', { id: id })
   .done(function(d) { if(d.success) location.hash='#/gallery_Dress'; else alert(d.message); });
}
</script>
<?php endif; ?></div>
